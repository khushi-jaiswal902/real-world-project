...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async(event) => {
    ...
    try {
      console.log("Submit handler triggered with paymentMethod:", paymentMethod);
      ...
      console.log("Cart validation result:", hasItems);
      if(!hasItems) {
      ...
      for(const items in cartItems) {
        for(const item in cartItems[items]) {
          if(cartItems[items][item] > 0) {
            const itemInfo = structuredClone(allProducts.find(product => product._id == items))
            if(itemInfo) {
              itemInfo.size = item
      ...
      console.log('DEBUG FRONTEND ORDER DATA:', JSON.stringify(orderData, null, 2));
      ...
      switch(paymentMethod) {
        ...
        case 'stripe':
            ...
            break;
        case 'razorpay':
            console.log("Razorpay block triggered. Token:", token);
            const responseRazorpay = await axios.post(backendUrl + '/api/order/razorpay', orderData, {headers: {token}})
            console.log("Razorpay post response:", responseRazorpay.data);
      ...
      console.error("Submit handler catch block error:", error);
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='mt-8'>
        ...
        <div className='w-full text-end mt-8'>
          <button type='submit' className='bg-black text-white text-sm my-8 px-16 py-3'>PROCEED TO PAY</button>
        </div>
      </div>
    </form>
  )
