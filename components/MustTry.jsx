...
import { mustTryList } from '../assets/assets'
import { ShopContext } from '../context/ShopContext'

const MustTry = () => {
    const currency = 'Rs'
    const { setSearch, setShowSearch, navigate, addToCart, toggleWishlist, isInWishlist } = useContext(ShopContext)
    ...
    return (
        <div className='my-16'>
            ...
            <div className='flex gap-6 overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory'>
                {mustTryList.map((item, index) => {
                    const image = item.image?.[0]
                    const inWishlist = isInWishlist ? isInWishlist(item._id) : false
                    const topLabel = index % 3 === 0 ? 'Trending' : 'BestSeller'
                    const topLabelColor = index % 3 === 0 ? 'bg-[#6f42c1]' : 'bg-[#2373d6]'

                    return (
                        <div
                            key={`${item._id}-must-try`}
                        ...
                        >
                            <div className='flex items-center justify-between bg-gradient-to-r from-[#dbeafe] via-white to-[#ffe0e6] px-4 py-2 text-xs font-semibold text-white'>
                                <span className={`rounded-full px-6 py-1 ${topLabelColor}`}>{topLabel}</span>
                            ...
                            <div className='px-4 pb-4 flex flex-col'>
                                ...
                                <p className='mb-3 text-sm text-slate-500 line-clamp-2 min-h-[40px]'>
                                    {item.description || 'Must-have pick loved for its standout results.'}
                                </p>
